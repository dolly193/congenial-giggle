...
def list_all_users():
    ...
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, username, is_approved, quota_used, is_vip, vip_expiration FROM users")
    users = cursor.fetchall()
    conn.close()
    return jsonify(users)
...
def dolly_store_feed():
    ...
    cursor.execute("""
        SELECT f.hash, f.filename, f.size_bytes, f.created_at, u.username, u.is_admin, u.is_vip 
...
def support_chat():
    ...
    # Lista de modelos para testar em ordem de prioridade (Fallback automático)
    # Tenta o 2.5 (solicitado), depois o 1.5 (padrão atual), depois o Pro (legado)
    models_to_try = ['gemini-2.5-flash', 'gemini-2.5-flash-001', 'gemini-1.5-flash', 'gemini-pro']
    last_error = None
